...
function select(i, j) {
  if (round < 8) {
    ...
    if (document.getElementById("box" + i + j).classList == "fab")
      if (round % 2 == 0)
        document.getElementById("box" + i + j).classList = "far fa-dot-circle";
      else
        document.getElementById("box" + i + j).classList = "fas fa-dot-circle";
    else round--;
    checkFilled();
  } else if (
    (round % 2 == 0 &&
      document.getElementById("box" + i + j).classList !=
        "far fa-dot-circle") ||
    (round % 2 == 1 &&
      document.getElementById("box" + i + j).classList != "fas fa-dot-circle")
  ) {
    if (document.getElementById("box" + i + j).classList != "fab") {
      ...
      document.getElementById("box" + i + j).classList == "fab" &&
      !move
    ) {
      if (
        (fixedi == i && (fixedj == j + 1 || fixedj == j - 1)) ||
        ...
        tempAlert(
          "இதை தேர்ந்தெடுக்க முடியாது. வேறு எதையும் தேர்ந்தெடுக்கவும்.\nCannot select this. Select any other key."
        );
        round++;
      }
      fixedi = 4;
...
function end() {
  switch (win) {
    case 1:
      {
        tempAlert(
          "முதலாம் ஆட்டக்காரர் வெற்றி பெற்று விட்டார்.\nPlayer 1 Wins."
        );
        reset();
      }
      break;
    case 2: {
      tempAlert("இரண்டாம் ஆட்டக்காரர் வெற்றி பெற்று விட்டார்.\nPlayer 2 Wins.");
      reset();
    }
  }
}
